...
### Чтение состояния из пина: `digitalRead(pin)`
...
- Тип таймеров unsigned long.
- Оператор «and» — это допустимый синоним «&&» в C++. Для индустрии чаще используют «&&».
- Комплексирование операторов digitalWrite(pin, !digitalRead(pin)) удобно для «переключения».
